...
ROOT = './../srVAE_orig/data_root/'


# ----- Dataset Splitter -----

def get_samplers(num_train, valid_size, generator):
...
def dataloader(dataset=args.dataset, data_root=ROOT, batch_size=args.batch_size, num_workers=6, pin_memory=True):
    ...
    # define samplers for obtaining training and validation batches
    train_sampler, valid_sampler = get_samplers(len(train_data), 0.15, generator)

    # Build dataloaders
    train_loader = DataLoader(train_data, sampler=train_sampler, generator=generator, **loader_kwargs)
    valid_loader = DataLoader(valid_data, sampler=valid_sampler, generator=generator, **loader_kwargs)
    test_loader  = DataLoader(test_data,  shuffle=False, generator=generator, **loader_kwargs)
